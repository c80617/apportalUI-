...
import brahma.vmi.covid2019.expandmenu.utils.DpOrPxUtils;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()){
            ...
            case MotionEvent.ACTION_UP:
                Log.d("IanIan","onTouchEvent ACTION_UP" );
                Log.d("IanIan","onTouchEvent buttonStyle = " + buttonStyle);
                Log.d("IanIan","onTouchEvent backPathWidth = " + backPathWidth + " maxBackPathWidth = " + maxBackPathWidth);
                if(backPathWidth==maxBackPathWidth || backPathWidth==0){//动画结束时按钮才生效
                    Log.d("IanIan","switch buttonStyle = " + buttonStyle);
                    switch (buttonStyle){
                        case Right:
                            if(y>=buttonTop&&y<=buttonBottom&&x>=rightButtonLeft&&x<=rightButtonRight){
                                expandMenu(expandAnimTime);
                            }
                            break;
                        case Left:
                            Log.d("IanIan","onTouchEvent Left" );
                            if(y>=buttonTop &&
                                    y<=buttonBottom &&
                                    x>=leftButtonLeft &&
                                    x<=leftButtonRight){
                                expandMenu(expandAnimTime);
                                Log.d("IanIan","onTouchEvent expandMenu" );
                            }
                            break;
                    }
                }
