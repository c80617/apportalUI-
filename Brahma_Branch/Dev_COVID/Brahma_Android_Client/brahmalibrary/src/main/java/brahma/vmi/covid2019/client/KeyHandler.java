...
import static brahma.vmi.covid2019.apprtc.CallActivity.getMediaStream;
import static brahma.vmi.covid2019.apprtc.CallActivity.handler;
...
public class KeyHandler implements Constants {
    ...
    private void setMediaVolume(int volume) {
        ...
        if(volume <= 0){//靜音
            ...
            if(notMute){//notMute = false -> 靜音;notMute = true -> 有聲音
                //audioMgr.setStreamVolume(AudioManager.STREAM_VOICE_CALL, volume, AudioManager.FLAG_SHOW_UI);
                audioMgr.setStreamVolume(AudioManager.STREAM_VOICE_CALL, volume, FLAG_PLAY_SOUND);
                //for show
                audioMgr.setStreamVolume(AudioManager.STREAM_MUSIC, fakeVolume, FLAG_SHOW_UI);
                //handler.sendEmptyMessage(volume);
            }else{
                //audioMgr.setStreamVolume(AudioManager.STREAM_VOICE_CALL, 1, AudioManager.FLAG_SHOW_UI);
                audioMgr.setStreamVolume(AudioManager.STREAM_VOICE_CALL, volume, FLAG_PLAY_SOUND);
                //for show
                audioMgr.setStreamVolume(AudioManager.STREAM_MUSIC, fakeVolume, FLAG_SHOW_UI);
                //handler.sendEmptyMessage(volume);
    ...
    public void setMediaVolumeOppsite(int volume) {
        ...
        int fakeVolume = volume;
        int orignVolume = (int)(volume*oppsiteRatio);
        Log.d("isKeyEvent","fakeVolume:"+fakeVolume);
