...
void gles2_unimplemented() {
    ...
    ERROR("Called unimplemented GLES API");
}
...
void *gles2_dispatch_get_proc_func(const char *name, void *userData)
{
    ...
    if (!func) {
        DEBUG("GLESv2Dispatch:: gles2 dispatch symbol NOT found: %s in %s", 
              name, DEFAULT_GLES_V2_LIB);
        func = (void *)gles2_unimplemented;
    } else    {
        DEBUG("GLESv2Dispatch:: gles2 dispatch symbol found: %s in %s",
              name, DEFAULT_GLES_V2_LIB);
    }
