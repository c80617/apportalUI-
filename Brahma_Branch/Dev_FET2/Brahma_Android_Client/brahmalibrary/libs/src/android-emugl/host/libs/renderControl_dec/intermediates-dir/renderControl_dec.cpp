...
size_t renderControl_decoder_context_t::decode(void *buf, size_t len, IOStream *stream, ChecksumCalculator* checksumCalc) {
	...
	while (end - ptr >= 8) {
		...
		case OP_rcWaitSyncKHR: {
			...
			DEBUG("renderControl(%p): rcWaitSyncKHR(0x%016lx 0x%08x )\n", stream, var_sync, var_flags);
		...
		case OP_rcCompose: {
			...
			totalTmpSize += checksumSize;
			unsigned char *tmpBuf = stream->alloc(totalTmpSize);
			DEBUG("renderControl(%p): rcCompose(0x%08x %p(%u) )\n", stream, var_bufferSize, (void*)(inptr_buffer.get()), size_buffer);
		...
		case OP_rcCreateDisplay: {
			...
			DEBUG("renderControl(%p): rcCreateDisplay(%p(%u) )\n", stream, (uint32_t*)(outptr_displayId.get()), size_displayId);
		...
		case OP_rcDestroyDisplay: {
			...
			totalTmpSize += checksumSize;
			unsigned char *tmpBuf = stream->alloc(totalTmpSize);
			DEBUG("renderControl(%p): rcDestroyDisplay(0x%08x )\n", stream, var_displayId);
		...
		case OP_rcSetDisplayColorBuffer: {
			...
			totalTmpSize += checksumSize;
			unsigned char *tmpBuf = stream->alloc(totalTmpSize);
			DEBUG("renderControl(%p): rcSetDisplayColorBuffer(0x%08x 0x%08x )\n", stream, var_displayId, var_colorBuffer);
		...
		case OP_rcGetDisplayColorBuffer: {
			...
			DEBUG("renderControl(%p): rcGetDisplayColorBuffer(0x%08x %p(%u) )\n", stream, var_displayId, (uint32_t*)(outptr_colorBuffer.get()), size_colorBuffer);
		...
		case OP_rcGetColorBufferDisplay: {
			...
			unsigned char *tmpBuf = stream->alloc(totalTmpSize);
			OutputBuffer outptr_displayId(&tmpBuf[0], size_displayId);
			DEBUG("renderControl(%p): rcGetColorBufferDisplay(0x%08x %p(%u) )\n", stream, var_colorBuffer, (uint32_t*)(outptr_displayId.get()), size_displayId);
		...
		case OP_rcGetDisplayPose: {
			...
			DEBUG("renderControl(%p): rcGetDisplayPose(0x%08x %p(%u) %p(%u) %p(%u) %p(%u) )\n", stream, var_displayId, (GLint*)(outptr_x.get()), size_x, (GLint*)(outptr_y.get()), size_y, (uint32_t*)(outptr_w.get()), size_w, (uint32_t*)(outptr_h.get()), size_h);
		...
		case OP_rcSetDisplayPose: {
			...
			totalTmpSize += checksumSize;
			unsigned char *tmpBuf = stream->alloc(totalTmpSize);
			DEBUG("renderControl(%p): rcSetDisplayPose(0x%08x 0x%08x 0x%08x 0x%08x 0x%08x )\n", stream, var_displayId, var_x, var_y, var_w, var_h);
		...
		case OP_rcSetColorBufferVulkanMode: {
			...
			totalTmpSize += checksumSize;
			unsigned char *tmpBuf = stream->alloc(totalTmpSize);
			DEBUG("renderControl(%p): rcSetColorBufferVulkanMode(0x%08x 0x%08x )\n", stream, var_colorBuffer, var_mode);
		...
		case OP_rcReadColorBufferYUV: {
			...
			unsigned char *tmpBuf = stream->alloc(totalTmpSize);
			OutputBuffer outptr_pixels(&tmpBuf[0], size_pixels);
			DEBUG("renderControl(%p): rcReadColorBufferYUV(0x%08x 0x%08x 0x%08x 0x%08x 0x%08x %p(%u) 0x%08x )\n", stream, var_colorbuffer, var_x, var_y, var_width, var_height, (void*)(outptr_pixels.get()), size_pixels, var_pixels_size);
