...
/* We're using Android NDK EGL/egl.h */
...
#define RETURN_ERROR(ret,err)                                  \
        ...
        if(tls_thread->getError() == EGL_SUCCESS) {            \
          tls_thread->setError(err);                           \
                                                               ...
                                                               \
        ERR("AEGL: RETURN_ERROR (%s:%d)", __FILE__, __LINE__); \
                                                               \
        return ret;
...
static bool createAndBindAuxiliaryContext(EGLContext* context_out, EGLSurface* surface_out) {
    ...
    if (!eglChooseConfig(dpy, configAttribs, &config, 1, &numConfigs) ||
        numConfigs == 0) {
        ERR("%s: could not find gles 2 config!\n", __func__);
        return false;
    }
...
static bool unbindAndDestroyAuxiliaryContext(EGLContext context, EGLSurface surface) {
    ...
    if (!eglMakeCurrent(
            ...
            EGL_NO_CONTEXT)) {
        ERR("%s: failure to unbind current context!\n", __func__);
        return false;
    }


    if (!eglDestroySurface(dpy, surface)) {
        ERR("%s: failure to destroy surface!\n", __func__);
        return false;
    }

    if (!eglDestroyContext(dpy, context)) {
        ERR("%s: failure to destroy context!\n", __func__);
        return false;
    }
...
EGLAPI EGLBoolean EGLAPIENTRY eglInitialize(EGLDisplay display, EGLint *major, EGLint *minor) {
    ...
    if(!g_eglInfo->getIface(GLES_1_1)) {
        ...
        if (func) {
            ...
            INFO("%s: Found ifaces for GLES CM 1.1 [%s]", __FUNCTION__, LIB_GLES_CM_NAME);
        } else {
            DBG("%s: Could not find ifaces for GLES CM 1.1 [%s]\n",
                __FUNCTION__, error);
            return EGL_FALSE;
        }
        initGLESx(GLES_1_1);
    ...
    if(!g_eglInfo->getIface(GLES_2_0)) {
        ...
        if (func) {
            INFO("%s: Found ifaces for GLES 2.0 [%s]", __FUNCTION__, LIB_GLES_V2_NAME);
            renderableType |= EGL_OPENGL_ES2_BIT;
            g_eglInfo->setIface(func(&s_eglIface),GLES_2_0);
        } else {
            DBG("%s: Could not find ifaces for GLES 2.0 [%s]\n",
                __FUNCTION__, error);
        }
        initGLESx(GLES_2_0);
    }
    if(!g_eglInfo->getIface(GLES_3_0)) {
        ...
        if (func) {
           ...
           DBG("%s: Could not find ifaces for GLES 3.x [%s]\n",
               __FUNCTION__, error);
        }
        initGLESx(GLES_3_0);
    }
...
#define CHOOSE_CONFIG_DLOG_BAD_ATTRIBUTE(attrname) \
            ...
            case EGL_NATIVE_VISUAL_ID:
                break; //we dont care from those selection criteria
            case EGL_LEVEL:
    ...
    EglConfig dummy(red_size,green_size,blue_size,alpha_size,caveat,conformant,depth_size,
                    ...
                    surface_type,transparent_type,trans_red_val,trans_green_val,trans_blue_val,
                    recordable_android, framebuffer_target_android,
                    NULL);
        ...
        while(attrib_list[i] != EGL_NONE) {
            ...
            case EGL_CONTEXT_FLAGS_KHR:
                if ((attrib_val | EGL_CONTEXT_OPENGL_DEBUG_BIT_KHR) ||
                    ...
                    ERR("%s: wrong context flags, return\n", __func__);
            ...
            case EGL_CONTEXT_OPENGL_PROFILE_MASK_KHR:
                if ((attrib_val | EGL_CONTEXT_OPENGL_CORE_PROFILE_BIT_KHR) ||
                    ...
                    ERR("%s: wrong profile mask, return\n", __func__);
                    RETURN_ERROR(EGL_NO_CONTEXT,EGL_BAD_ATTRIBUTE);
                }
                break;
            case EGL_CONTEXT_OPENGL_RESET_NOTIFICATION_STRATEGY_KHR:
                ...
                case EGL_LOSE_CONTEXT_ON_RESET_KHR:
                    break;
                default:
                    ERR("%s: wrong reset notif strat, return\n", __func__);
                    RETURN_ERROR(EGL_NO_CONTEXT,EGL_BAD_ATTRIBUTE);
                }
                reset_notification_strategy = attrib_val;
                break;
            default:
                ERR("%s: unknown attrib 0x%x\n", __func__, attrib_list[i]);
                RETURN_ERROR(EGL_NO_CONTEXT,EGL_BAD_ATTRIBUTE);
            }
...
EGLAPI EGLConfig EGLAPIENTRY eglLoadConfig(EGLDisplay display, EGLStream stream) {
    ...
    if (!cfg) {
        WARN("WARNING: EGL config mismatch, fallback to default configs\n");
