...
#define REPORT_ERROR()                                           \
    do {                                                         \
        ...
        if (err != GL_NO_ERROR) {                                \
            ERR("GLESv2Imp::%s: got err 0x%08x", __func__, err); \
        }                                                        \
     } while(0)
#endif
...
GL_APICALL void  GL_APIENTRY glBindTexture(GLenum target, GLuint texture){
    ...
    SET_ERROR_IF(!(GLESv2Validate::textureTarget(ctx, target) ||
                   GLESv2Validate::textureTargetEx(ctx, target)),
                 GL_INVALID_ENUM);
    ...
    if (ctx->shareGroup().get()) {
        ...
        if (ctx->GLTextureTargetToLocal(texData->target) != ctx->GLTextureTargetToLocal(target)) {
            ERR("GLESv2Imp::%s: Set invalid operation!\n", __func__);
        }
...
GL_APICALL void  GL_APIENTRY glCompileShader(GLuint shader){
    ...
    if(ctx->shareGroup().get()) {
        ...
        if (sp->validShader()) {
            ...
            ERR("GLESv2Imp::glCompileShader: invalid shader %d", shader);
            ctx->dispatcher().glCompileShader(globalShaderName);
            sp->setCompileStatus(false);
            ctx->dispatcher().glGetShaderiv(globalShaderName,GL_COMPILE_STATUS,&compileStatus);
            if (compileStatus != GL_FALSE) {
                ERR("%s: Warning: underlying GL compiled invalid shader!\n", __func__);
            }
        }
    }
}
...
#define VALIDATE(cond, err) do { if (cond) { *err_out = err; ERR("%s:%d failed validation\n", __FUNCTION__, __LINE__); return; } } while(0) \
...
GL_APICALL void  GL_APIENTRY glTexImage2D(GLenum target, GLint level, GLint internalformat, GLsizei width, GLsizei height, GLint border, GLenum format, GLenum type, const GLvoid* pixels){
    ...
    GLint err = ctx->dispatcher().glGetError();
    if (err != GL_NO_ERROR) {
        ERR("GLESv2Imp::%s: got err pre :( 0x%x internal 0x%x format 0x%x type 0x%x\n",
    ...
    if (err != GL_NO_ERROR) {
        ERR("%s: got err :( 0x%x, target 0x%0x, internal 0x%0x, format 0x%0x, type 0x%0x",
...
GL_APICALL void GL_APIENTRY glEGLImageTargetTexture2DOES(GLenum target, GLeglImageOES image)
{
    ...
    if (img) {
        ...
        if (ctx->shareGroup().get()) {
            ...
            if (!imagehndl) {
                ERR("glEGLImageTargetTexture2DOES with empty handle\n");
            }
        }
    }
}
...
static GLuint compileShader(GLDispatch* gl,
    ...
    if (compileStatus != GL_TRUE) {
        ...
        ERR("Failed to compile shader. Info log: [%s]\n", infoLog.data());
    }
...
static GLint compileAndLinkShaderProgram(GLDispatch* gl,
    ...
    if (linkStatus != GL_TRUE) {
        ...
        gl->glGetProgramInfoLog(program, infoLogLength, nullptr,
                            infoLog.data());
        ERR("Failed to link program. Info log: [%s]\n", infoLog.data());
    }
