...
void emugl_logger(const char* fmt, ...)    {
    va_list args;
    va_start(args, fmt);
...
void emugl_ctx_logger(const char* fmt, ...)    {
    va_list args;
    va_start(args, fmt);
    vfprintf(stderr, fmt, args);
    va_end(args);
...
//static emugl_logger_struct sEmuglLogFuncs = { emugl_logger, emugl_ctx_logger };
//static emugl_logger_struct sEmuglLogFuncs = { emugl_logger, nullptr };
...
static const QAndroidEmulatorWindowAgent sQAndroidEmulatorWindowAgent = {
        ...
        .fold =
                [](bool is_fold) {
                    printf("OpenglEsStreamHandler: .fold %d\n", is_fold);
                    sIsFolded = is_fold;
        ...
        .isFolded =
                [](void) -> bool {
                    printf("OpenglEsStreamHandler: .isFolded ? %d\n", sIsFolded);
                    return sIsFolded;
                },
        .setUIDisplayRegion =
                [](int x_offset, int y_offset, int w, int h) {
                    printf("OpenglEsStreamHandler: .setUIDisplayRegion %d %d %dx%d\n",
                           x_offset, y_offset, w, h);
                },
        .setMultiDisplay =
                [](uint32_t id, int32_t x, int32_t y, uint32_t w, uint32_t h, bool add) {
                    printf("OpenglEsStreamHandler: .setMultiDisplay id %d %d %d %dx%d %s\n",
        ...
        .switchMultiDisplay =
                [](bool add, uint32_t id, int32_t x, int32_t y, uint32_t w, uint32_t h,
                   uint32_t dpi, uint32_t flag) {
                    printf("OpenglEsStreamHandler: .switchMultiDisplay id %d %d %d %dx%d "
                           "dpi %d flag %d %s\n",
