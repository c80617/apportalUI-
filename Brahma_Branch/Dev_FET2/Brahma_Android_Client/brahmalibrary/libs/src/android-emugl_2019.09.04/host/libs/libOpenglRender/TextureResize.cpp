...
static GLuint createShader(GLenum type, std::initializer_list<const char*> source) {
    ...
    if (shader) {
        ...
        if (success == GL_FALSE) {
            ...
            ERR("TextureResize:: %s shader compile failed:\n%s\n",
...
static void attachShaders(TextureResize::Framebuffer* fb, const char* factorDefine,
    ...
    {
        GLint success;
        s_gles2.glGetProgramiv(fb->program, GL_LINK_STATUS, &success);
        if (success == GL_FALSE) {
            GLchar messages[256];
            s_gles2.glGetProgramInfoLog(fb->program, sizeof(messages), 0, &messages[0]);
            ERR("TextureResize::%s: Could not create/link program: %s", __FUNCTION__, messages);
        } else    {
            INFO("TextureResize::%s: Successfully create/link native gles program...", __FUNCTION__);
        }
    }
