...
public:
    EglOsGlLibrary() {
        ...
        if (!mLib) {
            ...
            INFO("%s: Finished loading open GL library %s...", __FUNCTION__, kGLES2LibName);
        }
    }
...
void EglOsEglDisplay::swapBuffers(Surface* surface) {
    ...
    DBG("EglOsEglDisplay::%s: mWin==%p, surface==%p\n", __FUNCTION__, sfc->getWin(), sfc->getHndl());
...
bool EglOsEglDisplay::checkWindowPixelFormatMatch(EGLNativeWindowType win,
    ...
    *height = ANativeWindow_getHeight((ANativeWindow *)win);
    ...
    D("%s: native window = %p, %d x %d", __FUNCTION__, win, *width, *height);
