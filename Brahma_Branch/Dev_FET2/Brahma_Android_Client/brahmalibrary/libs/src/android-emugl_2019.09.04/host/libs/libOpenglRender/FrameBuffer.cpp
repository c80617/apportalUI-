...
bool FrameBuffer::initialize(int width, int height, bool useSubWindow,
        bool egl2egl) {
    GL_LOG("FrameBuffer::initializing...");
    ...
    if (s_egl.eglUseOsEglApi)    {
        GL_LOG("eglUseOsEglApi == %s", egl2egl ? "true" : "false");
    ...
    GL_LOG("FrameBuffer:: call eglInitialize");
    ...
    DBG("FrameBuffer:: egl: %d %d", fb->m_caps.eglMajor, fb->m_caps.eglMinor);
    GL_LOG("FrameBuffer:: egl: %d %d", fb->m_caps.eglMajor, fb->m_caps.eglMinor);
    ...
    DBG("FrameBuffer:: gles version: %d, %d %d\n", dispatchMaxVersion, glesMaj, glesMin);
    GL_LOG("FrameBuffer:: gles version: %d %d\n", glesMaj, glesMin);
    ...
    // temporarily disable fast blit support for Android ...
    fb->m_asyncReadbackSupported = false;

    // Android aarch64 support both bast blit and non-fast blit
    // but... when running temple run, TextureDraw blitFromCurrentReadBuffer
    // "s_gles2.glBindTexture(GL_TEXTURE_2D, currTexBind);" will have some issues
    // if fast blit is disabled... 
    //fb->disableFastBlit();
#endif
    ...
    EGLint wantedAlphaSize = 8;
    EGLint wantedDepthSize = 24;
#if (defined(ANDROID) || defined(__ANDROID__))
    const GLint configAttribs[] = {
             EGL_RED_SIZE, wantedRedSize, EGL_GREEN_SIZE, wantedGreenSize,
             EGL_BLUE_SIZE, wantedBlueSize, EGL_ALPHA_SIZE, wantedAlphaSize,
             EGL_DEPTH_SIZE, wantedDepthSize, EGL_SURFACE_TYPE, surfaceType,
    ...
    for (EGLint i = 0; i < total_egl_compatible_configs; i++) {
        ...
        s_egl.eglGetConfigAttrib(fb->m_eglDisplay, c, EGL_DEPTH_SIZE, &z);

        if (r == wantedRedSize && g == wantedGreenSize && b == wantedBlueSize &&
            a == wantedAlphaSize && z == wantedDepthSize) {
            exact_match_index = i;
            break;
        }
    }
    ...
    GL_LOG("FrameBuffer::%s: attempting to create egl context, EglConfig(%p)",
           __FUNCTION__, fb->m_eglConfig);
    ...
    GL_LOG("FrameBuffer::%s: attempting to create egl pbuffer context", __FUNCTION__);
    //
...
bool FrameBuffer::setupSubWindow(FBNativeWindowType p_window,
                                 ...
                                 float zRot,
                                 bool deleteExisting) {
    GL_LOG("FrameBuffer:: Begin setupSubWindow, deleteExisting == %s",
           deleteExisting ? "true" : "false");
    if (!m_useSubWindow) {
...
#if (defined(ANDROID) || defined(__ANDROID__))
        EGLint format;
        s_egl.eglGetConfigAttrib(m_eglDisplay, m_eglConfig , EGL_NATIVE_VISUAL_ID, &format);
        m_subWin = ::createSubWindow(format, m_x, m_y, m_windowWidth,
                                     m_windowHeight, subWindowRepaint, this);
        DBG("In setupSubWindow: m_subWin == %p, m_eglConfig == %p, EGL_NATIVE_VISUAL_ID == %d",
            m_subWin, m_eglConfig, format);

#else
        m_subWin = ::createSubWindow(p_window, m_x, m_y, m_windowWidth,
                                     m_windowHeight, subWindowRepaint, this);
        DBG("FrameBuffer:: In setupSubWindow: m_subWin == %p", m_subWin);
#endif
...
HandleType FrameBuffer::createWindowSurface(int p_config,
    ...
    INFO("FrameBuffer::createWindowSurface: %d, %dx%d", p_config, p_width, p_height); 
    ...
    INFO("FrameBuffer::createWindowSurface: ret == %d", ret);
...
void FrameBuffer::closeColorBufferLocked(HandleType p_colorbuffer,
    ...
    /** Sting: if  android::featurecontrol::RefCountPipe is not set to true,
                  we'll have colour not found problem here */  
...
bool FrameBuffer::bindContext(HandleType p_context,
    ...
    if (!s_egl.eglMakeCurrent(m_eglDisplay,
        ...
        ERR("FrameBuffer::%s: eglMakeCurrent failed", __FUNCTION__);
        return false;
    }
...
bool FrameBuffer::bind_locked() {
    ...
    if (prevContext != m_pbufContext || prevReadSurf != m_pbufSurface ||
        ...
        ERR("FrameBuffer:: Nested %s call detected, should never happen\n", __func__);
    }
...
bool FrameBuffer::bindSubwin_locked() {
    ...
    if (prevContext != m_eglContext || prevReadSurf != m_eglSurface ||
        ...
        if (!s_egl.eglMakeCurrent(m_eglDisplay, m_eglSurface, m_eglSurface,
                                  m_eglContext)) {
            ERR("FrameBuffer::%s: eglMakeCurrent failed in binding subwindow!", __FUNCTION__);
            return false;
        } else    {
            DBG("FrameBuffer::%s: eglMakeCurrent successfully in binding subwindow!", __FUNCTION__);
        }
    }
...
void FrameBuffer::sting_report()    {
    printf("    m_colorbuffers(%lu):", m_colorbuffers.size());
...
bool FrameBuffer::postImpl(HandleType p_colorbuffer,
    ...
    ColorBufferMap::iterator c(m_colorbuffers.find(p_colorbuffer));
    if (c == m_colorbuffers.end()) {
        ERR("FrameBuffer::%s: unable to find p_colorbuffer(%#x:%s:%s), exiting...",
             __func__, p_colorbuffer,
             repaint ? "repaint" : "NO-repaing",
             m_refCountPipeEnabled ? "true" : "false");
        //sting_report();
    ...
    if (m_displays.size() != 1 || m_displays.find(0) == m_displays.end()) {
        ...
        for (const auto& iter : m_displays) {
            emugl::get_emugl_window_operations().setMultiDisplay(
                iter.first,
                iter.second.pos_x,
                combinedDisplayHeight - iter.second.pos_y - iter.second.height,
                iter.second.width,
 ...
 * by invoking setMultiDisplay() will be using top-left corner as origin. Thus,
