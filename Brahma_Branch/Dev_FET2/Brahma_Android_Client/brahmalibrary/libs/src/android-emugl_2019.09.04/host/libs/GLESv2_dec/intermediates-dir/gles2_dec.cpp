...
#  define fprintf(io, ...) DEBUG(__VA_ARGS__)
...
size_t gles2_decoder_context_t::decode(void *buf, size_t len, IOStream *stream, ChecksumCalculator* checksumCalc) {
	...
	while (end - ptr >= 8) {
		...
		#endif
			DEBUG("gles2(%p): glMapBufferRangeDMA(0x%08x 0x%08lx 0x%08lx 0x%08x 0x%016lx )\n", stream, var_target, var_offset, var_length, var_access, var_paddr);
		...
		#endif
			DEBUG("gles2(%p): glUnmapBufferDMA(0x%08x 0x%08lx 0x%08lx 0x%08x 0x%016lx %p(%u) )\n", stream, var_target, var_offset, var_length, var_access, var_paddr, (GLboolean*)(outptr_out_res.get()), size_out_res);
		...
		#endif
			DEBUG("gles2(%p): glMapBufferRangeDirect(0x%08x 0x%08lx 0x%08lx 0x%08x 0x%016lx )\n", stream, var_target, var_offset, var_length, var_access, var_paddr);
		...
		#endif
			DEBUG("gles2(%p): glUnmapBufferDirect(0x%08x 0x%08lx 0x%08lx 0x%08x 0x%016lx 0x%016lx %p(%u) )\n", stream, var_target, var_offset, var_length, var_access, var_paddr, var_guest_ptr, (GLboolean*)(outptr_out_res.get()), size_out_res);
		...
		#endif
			DEBUG("gles2(%p): glFlushMappedBufferRangeDirect(0x%08x 0x%08lx 0x%08lx 0x%08x )\n", stream, var_target, var_offset, var_length, var_access);
		...
		#endif
			DEBUG("gles2(%p): glGetGraphicsResetStatusEXT()\n", stream);
		...
		#endif
			DEBUG("gles2(%p): glReadnPixelsEXT(%d %d %d %d 0x%08x 0x%08x %d %p(%u) )\n", stream, var_x, var_y, var_width, var_height, var_format, var_type, var_bufSize, (GLvoid*)(outptr_data.get()), size_data);
		...
		#endif
			DEBUG("gles2(%p): glGetnUniformfvEXT(%u %d %d %p(%u) )\n", stream, var_program, var_location, var_bufSize, (GLfloat*)(outptr_params.get()), size_params);
		...
		#endif
			DEBUG("gles2(%p): glGetnUniformivEXT(%u %d %d %p(%u) )\n", stream, var_program, var_location, var_bufSize, (GLint*)(outptr_params.get()), size_params);
		...
		#endif
			DEBUG("gles2(%p): glDrawArraysNullAEMU(0x%08x %d %d )\n", stream, var_mode, var_first, var_count);
		...
		#endif
			DEBUG("gles2(%p): glDrawElementsNullAEMU(0x%08x %d 0x%08x %p(%u) )\n", stream, var_mode, var_count, var_type, (const GLvoid*)(inptr_indices.get()), size_indices);
		...
		#endif
			DEBUG("gles2(%p): glDrawElementsOffsetNullAEMU(0x%08x %d 0x%08x %u )\n", stream, var_mode, var_count, var_type, var_offset);
		...
		#endif
			DEBUG("gles2(%p): glDrawElementsDataNullAEMU(0x%08x %d 0x%08x %p(%u) %u )\n", stream, var_mode, var_count, var_type, (void*)(inptr_data.get()), size_data, var_datalen);
