...
bool WindowSurface::flushColorBuffer() {
    ...
    if (mAttachedColorBuffer->getWidth() != mWidth ||
        ...
        ERR("WindowSurface::%s: Dimensions do not match", __FUNCTION__);
        return false;
    }

    if (!mDrawContext.get()) {
        ERR("WindowSurface::%s: Draw context is NULL", __FUNCTION__);
        return false;
    }
    ...
    if (needToSet) {
        if (!s_egl.eglMakeCurrent(mDisplay,
            ...
            ERR("WindowSurface::%s: Error making draw context current", __FUNCTION__);
            return false;
        }
    }

    if(!mAttachedColorBuffer->blitFromCurrentReadBuffer())    {
        ERR("WindowSurface::%s: Error blit from current read buffer ", __FUNCTION__);
        return false;
    }
...
bool WindowSurface::resize(unsigned int p_width, unsigned int p_height)
{
    ...
    if (mSurface == EGL_NO_SURFACE) {
        ERR("WindowSurface::%s: Renderer error: failed to create/resize pbuffer!!", __FUNCTION__);
        return false;
    }
