...
void gles1_unimplemented() {
    ERROR("Called unimplemented GLESv1 API");
}
...
bool gles1_dispatch_init(GLESv1Dispatch* dispatch_table) {
    ...
    if (!strcmp(libName, "<gles2_only_backend>")) {
        ...
        DEBUG("assigning dummies because <gles2_only_backend>");
        return true;
    } else {
...
#define SET_UNDERLYING_GLES2_FUNC(rett, function_name, sig, callargs) do { \
    dispatch_table->underlying_gles2_api->angle-> function_name = \
        reinterpret_cast< function_name ## _t >( \
            ...
            DEBUG("trying to initialize GLESv1->2 translation");
            ...
            if (!s_underlying_gles2_lib) {
                DEBUG("Could not load underlying gles2 lib %s [%s]",
                      underlying_gles2_lib_name, error);
                return false;
            }
            DEBUG("done trying to get gles2 lib");
...
void *gles1_dispatch_get_proc_func(const char *name, void *userData)
{
    ...
    if (!func) {
        DEBUG("GLESv1Dispatch:: gles1 dispatch symbol NOT found: %s in %s",
              name, DEFAULT_GLES_CM_LIB);
        func = (void *)gles1_unimplemented;
    } else    {
        DEBUG("GLESv1Dispatch:: gles1 dispatch symbol found: %s in %s",
              name, DEFAULT_GLES_CM_LIB);
    }
