...
void *GLESv2Decoder::s_getProc(const char *name, void *userData)
{
    ...
    DEBUG("GLESv2Decoder::s_getProc: %s, %p, %p", name, ctx, ctx->m_GL2library);
...
void GLESv2Decoder::s_glGetCompressedTextureFormats(void *self, int count, GLint *formats)
{
    ...
    if (nFormats > count) {
        ERR("%s: GetCompressedTextureFormats: The requested number of formats does not match the number that is reported by OpenGL\n", __FUNCTION__);
    } else {
