...
import brahma.vmi.brahmalibrary.performance.MeasurementInfo;
import brahma.vmi.brahmalibrary.performance.PointPerformanceData;
import brahma.vmi.brahmalibrary.performance.SpanPerformanceData;
...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    public static final String DB_NAME = "itri.vmi.brahmalibrary.db";
    ...
    public synchronized void close() {
        ...
        Log.d("DatabaseHandler","dbhandler close");
        AndroidLogger.d("DatabaseHandler","dbhandler close");
        // cleanup
        if (db != null && db.isOpen() && decrementAndGet == 0){
            Log.d("DatabaseHandler","dbhandler decrementAndGet close");
            AndroidLogger.d("DatabaseHandler","dbhandler decrementAndGet close");
            db.close();
        }
        decrementAndGet--;
    }
    ...
    public void onCreate(SQLiteDatabase db) {
        Log.d("DatabaseHandler","dbhandler onCreate");
        AndroidLogger.d("DatabaseHandler","dbhandler onCreate");
    ...
    public void onOpen(SQLiteDatabase db) {
        ...
        Log.d("DatabaseHandler","dbhandler onOpen");
        AndroidLogger.d("DatabaseHandler","dbhandler onOpen");
        super.onOpen(db);
        if (!db.isReadOnly()) {
    ...
    private void createTable(int tableID, SQLiteDatabase db) {
        ...
        AndroidLogger.d(TAG, String.format("Creating table: %s", query.toString()));
    ...
    public SessionInfo getSessionInfo(ConnectionInfo connectionInfo) {
        ...
        if (cursor.moveToFirst()) {
            ...
            if (token != null && token.length() > 0) {
                ...
                Log.v(TAG, String.format("Found session info, [token: '%s', expires: '%s', host: '%s', port: '%s']",token, sdf.format(expireDate), host, port));
                AndroidLogger.v(TAG, String.format("Found session info, [token: '%s', expires: '%s', host: '%s', port: '%s']",token, sdf.format(expireDate), host, port));
                ...
                if (expireDate.after(new Date())) {
                    ...
                    try {
                        ...
                        AndroidLogger.e(TAG, "Session info contained invalid webrtc JSON string:"+ e);
                    }
                }
                else {
